...
static void twai_transmit_task(void *arg)
{
    while (1)
    {
        ...
        if (action == TX_SEND_PING_RESP)
        {
            ...
            ESP_LOGI(CTRL_TAG, "Transmitted ping response");
            xSemaphoreGive(ctrl_task_sem);
        }
        else if (action == TX_SEND_DATA)
        {
            // Transmit data messages until stop command is received
            ESP_LOGI(CTRL_TAG, "Start transmitting data");
            while (1)
            {
                ...
                ESP_LOGI(CTRL_TAG, "Transmitted data value %" PRIu32, sensor_data);
        ...
        else if (action == TX_SEND_STOP_RESP)
        {
            ...
            ESP_LOGI(CTRL_TAG, "Transmitted stop response");
            xSemaphoreGive(ctrl_task_sem);
        }
