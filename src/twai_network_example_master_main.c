        ...
        if (action == RX_RECEIVE_PING_RESP) {
            ...
            while (data_msgs_rec < NO_OF_DATA_MSGS) {
                ...
                if (rx_msg.identifier == ID_SLAVE_DATA) {
                    ...
                    ESP_LOGI(CTRL_TAG, "Received data value %"PRIu32, data);
                    data_msgs_rec ++;
                }
            }
            xSemaphoreGive(ctrl_task_sem);
...
static void twai_transmit_task(void *arg)
{
    while (1) {
        ...
        if (action == TX_SEND_PINGS) {
            ...
            ESP_LOGI(CTRL_TAG, "Transmitting ping");
...
static void twai_control_task(void *arg)
{
    ...
    for (int iter = 0; iter < NO_OF_ITERS; iter++) {
        ...
        ESP_LOGI(CTRL_TAG, "Driver started");
        ...
        ESP_LOGI(CTRL_TAG, "Driver stopped");
...
void app_main(void)
{    
    // Add short delay to allow slave to initialize first
    for (int i = 4; i > 0; i--)
    {
        printf("Master starting in %d\n", i);
    ...
    ESP_LOGI(CTRL_TAG, "Driver installed");
    ...
    ESP_LOGI(CTRL_TAG, "Driver uninstalled");
